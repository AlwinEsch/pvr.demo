...
public:
   ...
   uint16_t TwoBytes(const int index) const { return data_->data ? data_->TwoBytes(off+index) : uint16_t(0); }
   uint32_t FourBytes(const int index) const { return data_->data ? data_->FourBytes(off+index) : uint32_t(0); }
...
private:
   ...
   public:
      ...
      uint16_t TwoBytes(const int index) const
         { return uint16_t((data[index] << 8) | data[index+1]); }
      uint32_t FourBytes(const int index) const
         { return uint32_t((data[index] << 24) | (data[index+1] << 16) | (data[index+2] << 8) | data[index+3]); }
...
class CRC32 {
public:
   CRC32(const char *d, int len, uint32_t CRCvalue=0xFFFFFFFF);
   bool isValid() { return crc32(data, length, value) == 0; }
   static bool isValid(const char *d, int len, uint32_t CRCvalue=0xFFFFFFFF) { return crc32(d, len, CRCvalue) == 0; }
   static uint32_t crc32(const char *d, int len, uint32_t CRCvalue);
protected:
