    ...
    if (m_RDSEnabled)
    {
      ...
      if (m_PesBuffer[p+m_FrameSize-1] == 0xfd && rdsl > 0)     // RDS DataSync = 0xfd @ end
      {
        if (m_RDSBuffer == NULL)
        {
          ...
          if (m_RDSBuffer == NULL)
          {
            XBMC->Log(LOG_ERROR, "PVR Parser MPEG2-Audio - %s - malloc failed for RDS data", __FUNCTION__);
            m_RDSEnabled = false;
            return;
          }
          ...
          if (!m_RDSExtPID)
          {
            XBMC->Log(LOG_ERROR, "PVR Parser MPEG2-Audio - %s - failed to add RDS data stream", __FUNCTION__);
            m_RDSEnabled = false;
            return;
          }
        }
        ...
        if (rdsl >= m_RDSBufferSize)
        {
          ...
          {
            XBMC->Log(LOG_ERROR, "PVR Parser MPEG2-Audio - %s - max buffer size (%i kB) for RDS data reached, pid: %d", __FUNCTION__, MAX_RDS_BUFFER_SIZE/1024, m_pID);
            m_RDSEnabled = false;
            return;
          }
          ...
          if (m_RDSBuffer == NULL)
          {
            XBMC->Log(LOG_ERROR, "PVR Parser MPEG2-Audio - %s - realloc for RDS data failed", __FUNCTION__);
            m_RDSEnabled = false;
            return;
          }
        }
