...
int cParser::ParsePacketHeader(uint8_t *data)
{
  ...
  {
    XBMC->Log(LOG_DEBUG, "no payload, size %d", bytes);
    return 0;
  }
...
bool cParser::AddPESPacket(uint8_t *data, int size)
{
  ...
  if (m_IsPusi)
  {
    ...
    else if (m_PesHeaderPtr)
    {
      ...
      {
        XBMC->Log(LOG_INFO, "error parsing pes packet error ");
        Reset();
        return false;
      }
      m_PesHeaderPtr = 0;
    ...
    else
    {
      ...
      {
        XBMC->Log(LOG_INFO, "error parsing pes packet error 2");
        Reset();
        return false;
      }
      data += hdr_len;
  ...
  if (m_PesBuffer == NULL)
  {
    ...
    if (m_PesBuffer == NULL)
    {
      XBMC->Log(LOG_ERROR, "cParser::AddPESPacket - malloc failed");
      Reset();
      return false;
    }
  }
  ...
  if (m_PesBufferPtr + size >= m_PesBufferSize)
  {
    ...
    {
      XBMC->Log(LOG_ERROR, "cParser::AddPESPacket - max buffer size reached, pid: %d", m_pID);
      Reset();
      return false;
    }
    ...
    if (m_PesBuffer == NULL)
    {
      XBMC->Log(LOG_ERROR, "cParser::AddPESPacket - realloc failed");
      Reset();
      return false;
    }
  }
  ...
  , m_pID(pid)
{
  ...
  {
    XBMC->Log(LOG_ERROR, "Unrecognised type %i inside stream %i", m_streamType, m_pID);
    return;
  }
}
...
void cTSStream::GetVideoInformation(int &FpsScale, int &FpsRate, int &Height, int &Width, float &Aspect)
{
  FpsScale = m_FpsScale;
  FpsRate = m_FpsRate;
  Height = m_Height;
...
void cTSStream::GetAudioInformation(int &Channels, int &SampleRate, int &BitRate, int &BitsPerSample, int &BlockAlign)
{
  Channels = m_Channels;
