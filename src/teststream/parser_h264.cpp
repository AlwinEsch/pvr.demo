...
void cParserH264::Parse(sStreamPacket *pkt, sStreamPacket *pkt_side_data)
{
  ...
  if (frameComplete)
  {
    if (!m_NeedSPS && !m_NeedIFrame && m_FrameValid)
    {
      ...
      XBMC->Log(LOG_DEBUG, "H.264 SPS: PAR %i:%i", m_PixelAspect.num, m_PixelAspect.den);
      XBMC->Log(LOG_DEBUG, "H.264 SPS: DAR %.2f", DAR);
...
bool cParserH264::Parse_SPS(uint8_t *buf, int len)
{
  ...
  XBMC->Log(LOG_DEBUG, "H.264 SPS: pic_width:  %u mbs", (unsigned) m_Width);
  XBMC->Log(LOG_DEBUG, "H.264 SPS: pic_height: %u mbs", (unsigned) m_Height);
  XBMC->Log(LOG_DEBUG, "H.264 SPS: frame only flag: %d", frame_mbs_only);
  ...
  if (!frame_mbs_only)
  {
    if (bs.readBits1())     /* mb_adaptive_frame_field_flag */
      XBMC->Log(LOG_DEBUG, "H.264 SPS: MBAFF");
  }
  ...
  if (bs.readBits1())       /* frame_cropping_flag */
  {
    ...
    XBMC->Log(LOG_DEBUG, "H.264 SPS: cropping %d %d %d %d", crop_left, crop_top, crop_right, crop_bottom);
  ...
  if (bs.readBits1())    /* vui_parameters_present flag */
  {
    if (bs.readBits1())  /* aspect_ratio_info_present */
    {
      ...
      XBMC->Log(LOG_DEBUG, "H.264 SPS: aspect_ratio_idc %d", aspect_ratio_idc);

      if (aspect_ratio_idc == 255 /* Extended_SAR */)
      {
        m_PixelAspect.num = bs.readBits(16); /* sar_width */
        m_PixelAspect.den = bs.readBits(16); /* sar_height */
        XBMC->Log(LOG_DEBUG, "H.264 SPS: -> sar %dx%d", m_PixelAspect.num, m_PixelAspect.den);
      }
      else
      {
        ...
        if (aspect_ratio_idc < sizeof(aspect_ratios)/sizeof(aspect_ratios[0]))
        {
          ...
          XBMC->Log(LOG_DEBUG, "H.264 SPS: PAR %d / %d", m_PixelAspect.num, m_PixelAspect.den);
        }
        else
        {
          XBMC->Log(LOG_DEBUG, "H.264 SPS: aspect_ratio_idc out of range !");
        }
      }
    }
...
//        m_FPS = time_scale / (num_units_in_tick * 2);
  ...
  XBMC->Log(LOG_DEBUG, "H.264 SPS: -> video size %dx%d, aspect %d:%d", m_Width, m_Height, m_PixelAspect.num, m_PixelAspect.den);
  return true;
}
